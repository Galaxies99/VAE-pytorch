...
class VAE(nn.Module):
    def __init__(self, image_size, in_channels, **kwargs):
        ...
        encoder_layer_num = len(self.hidden_dim)
        zipped_size = 2 ** encoder_layer_num
        ...
        if self.image_H % zipped_size != 0 or self.image_W % zipped_size != 0:
            raise AttributeError('The size of image should be divided by {}'.format(zipped_size))
        ...
        flatten_H = self.image_H // zipped_size
        flatten_W = self.image_W // zipped_size
        flatten_size = flatten_H * flatten_W

        self.encoder = nn.Sequential(
            *encoder_layers, 
            Rearrange('b c h w -> b (c h w)')
        )
        self.mu = nn.Linear(self.hidden_dim[-1] * flatten_size, self.latent_dim)
        self.log_var = nn.Linear(self.hidden_dim[-1] * flatten_size, self.latent_dim)
        ...
        self.decoder = nn.Sequential(
            nn.Linear(self.latent_dim, self.hidden_dim[-1] * flatten_size),
            Rearrange('b (c h w) -> b c h w', h = flatten_H, w = flatten_W),
            *decoder_layers
        )
